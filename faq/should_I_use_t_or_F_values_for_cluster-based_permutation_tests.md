...
Cluster-based permutation tests have originally beed used with a t-test statistic (Maris & Oostenveld, 2007, in J. of Neurosci. Methods) but their implementation in fieldtrip allows to use any other statistic function in addition to the common [depsamplesT](/reference/depsamplesT) and [indepsamplesT](/reference/indepsamplesT), such as e.g. [ft_statfun_depsamplesFunivariate](/reference/ft_statfun_depsamplesFunivariate) and [ft_statfun_indepsamplesF](/reference/ft_statfun_indepsamplesF). Using F values instead of t values for cluster-based permutation tests looks like a good opportunity to test for interaction effects and one might think that the results are equivalent to their t-test version. This is, however, not the case as will be explained below and it will finally be recommended to use the t statistic wherever possible in order to maintain the statistical properties of the test (in terms of power).
...
[ALTERNATIVE LINK WORKS BETTER?](/tutorial/cluster_permutation_timelock/#cluster-based-permutation-tests-on-event-related-fields)
