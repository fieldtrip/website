...
{% include markup/red %}
The `neuromag2ft` interface has some issues with the MEGIN TRIUX neo due to MEGIN DACQ software changes. Lauri Parkkonen has released a Lab Streaming Level (LSL) version of the real-time MEG interface for the MEGIN Triux neo system, which is now in use at Aalto University, Tsinghua University (Beijing), and ECNU (Shanghai). We recommend that over the `neuromag2ft` interface.
{% include markup/end %}
