...
The **[peercontroller](https://github.com/fieldtrip/fieldtrip/blob/release/peercontroller.m)** command will start the network buffer and the peer discovery threads in the background and signal the other peers on the network that you are not willing to execute jobs for them.
...
The **[peerworker](https://github.com/fieldtrip/fieldtrip/blob/release/peerworker.m)** command will start the network buffer and the peer discovery threads in the background, switch to worker mode to signal the other peers that it is willing to execute a job and wait until a request for execution comes in.
...
### Command line peerworkers that do not need MATLAB
