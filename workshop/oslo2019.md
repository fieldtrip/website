...
RITMO Centre affiliates have priority. There will be room for ~30 participants. RITMO centre employees and students have priority. When the local organizers have assessed how many RITMO affiliates are attending, the workshop will be open to external participants if there are still places left.
...
1.  Download all the (non-zip) files to the same path - that means that you need to put the mat-files from different sub-folders (in the ftp) into the same folder on your own computer
2.  Unzip the fieldtrip.zip file to the folder where you keep your MATLAB packages
3.  Unzip the dicom files into a folder called dicom which is on the path where you keep the other data files (step 1) 
