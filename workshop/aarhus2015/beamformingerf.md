...
Computing event-related fields with [MNE](/tutorial/source/minimumnormestimate) or frequency domain beamformer [DICS](/tutorial/source/beamforming) might be of interest. More information on [common filters can be found here](/example/source/beamformer_commonfilter). If you are doing a group study where you want the grid points to be the same over all subjects, [see here](/example/source/sourcemodel_aligned2mni). See [here for source statistics](/example/stats/source_statistics).
