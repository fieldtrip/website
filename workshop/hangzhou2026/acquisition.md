...
OPMs allow for flexible placement of sensors close to or directly on the scalp. Contrary to SQUID systems, OPM systems do not require a fixed helmet full of sensors. Since OPMs allow for this flexibility, many labs are experimenting with a relatively small number of OPM sensors. Whereas a full-head MEG system has the sensors distributed evenly around the whole head, with a small OPM system you can and should consider what the most optimal placement of the few channels is that you have available.

In this hands-on session we plan the acquisition of OPM-MEG data. We do not record any actual data, but we design the measurement setup: we make a head model of the (template) participant, design an OPM helmet with sensors at the positions of the extended 10-20 system, select a subset of the sensor positions corresponding to the number of OPMs that is available, and compute and plot the sensitivity of the resulting sensor array. The sensitivity map shows which brain regions the sensor array is sensitive to, and hence which placement of a small set of OPM sensors best matches your research question.
...
We use the standard colin27 template MRI that is included with FieldTrip. All procedures work in exactly the same way with an individual anatomical MRI of your own individual participant, with the main difference that the individual MRI then needs to be read in and the anatomical landmarks need to be determined.

### Download the sensor model
...
- reading and visualizing an anatomical MRI with **[ft_read_mri](/reference/fileio/ft_read_mri)** and **[ft_sourceplot](/reference/ft_sourceplot)**
- segmenting the anatomical MRI with **[ft_volumesegment](/reference/ft_volumesegment)**
- making a mesh of the head surface with **[ft_prepare_mesh](/reference/ft_prepare_mesh)**
- visualize the head surface mesh together with the cortical sheet and anatomical labeling, using **[ft_read_headshape](/reference/fileio/ft_read_headshape)** and **[ft_read_atlas](/reference/fileio/ft_read_atlas)**
- making an individual OPM helmet and placing OPM sensors according to the 10-20 system with **[ft_electrodeplacement](/reference/ft_electrodeplacement)** and **[ft_sensorplacement](/reference/ft_sensorplacement)**
- selecting a subset of OPM sensors with **[ft_electrodeselection](/reference/ft_electrodeselection)** and compute sensitivity maps with **[ft_prepare_leadfield](/reference/ft_prepare_leadfield)**

## Processing the anatomical MRI

### Reading and visualizing the anatomical MRI

We read the colin27 template MRI, which is a high-quality average MRI of a single person's brain, created by combining 27 scans of the same individual and aligning them to the MNI152 atlas. This is a widely used template, among others in SPM, and many anatomical atlasses can be combined with it.
...
### Visualize the head with the cortex and atlas labels
...
We plot the sensors on the helmet. We could also plot the OPM sensor holders and/or the holes that need to be made in the 3D printed helmet to glue the holders in.
...
In this tutorial we are assuming that we have an OPM system with only a limited number of OPM sensors that we want to place optimally for a specific research question, for example over a specific brain region. The selection of channels is done with **[ft_electrodeselection](/reference/ft_electrodeselection)**, which works both on the channel labels and on the channel indices.
...
{% include markup/yellow %}
For the FieldLine v3 OPM sensors it is recommended to only work with the two orientations `by` and `bz`, since the sensitivity of the `bx` channel is along the direction of the laser and is very noisy. In the design above we only work with the `_z` channels, which correspond to the radial `bz` orientation.
{% include markup/end %}
