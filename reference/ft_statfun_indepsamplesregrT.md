...
Note that this reference documentation is identical to the help that is displayed in MATLAB when you type "help ft_statfun_indepsamplesregrT".

`<html>``<pre>`
    `<a href=/reference/ft_statfun_indepsamplesregrT>``<font color=green>`FT_STATFUN_INDEPSAMPLESREGRT`</font>``</a>` calculates independent samples regression coefficient
