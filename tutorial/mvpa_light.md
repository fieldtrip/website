...
There are several possible ways in which to classify time-frequency data.
First, one can consider each point in the time-frequency plane separately, using
channels as features. The result
is a matrix of classification performances
 that can be plotted as an image in the time-frequency plane. Alternatively, classification
 can be performed only along the time dimension. In this case, for every time point, all  frequencies and channels together form the feature vector (of length #frequencies x #channels). The result is a vector of classification performances than can be plotted as a function of time (line plot). Finally, we can reverse the role of time and frequency and perform
 classification only along the frequency dimension. In this case, for every frequency bin, all  time points and channels together form the feature vector (of length #time points x #channels). The result is a vector of classification performances than can be plotted as a function of the frequency (line plot).
Which of these analyses is the most reasonable is ultimately
determined by the research question, but it may be useful to run all of these analyses
