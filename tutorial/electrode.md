...
In the next step we will transform our mesh into [CTF coordinates](/faq/how_are_the_different_head_and_mri_coordinate_systems_defined/). For this we have to specify the nasion (NAS), left preauricular (LPA) and right preauricular (RPA) points. See video for how to select fiducials for Matlab 2018 and earlier version (warning: in the video the order of fiducial selection is different). Note for Matlab 2019 and later: the procedure for 3-D image rotation has changed. For recent Matlab versions, first select fiducial on head, then select electrode label ("1" for the first fiducial), then enable rotate, then rotate the head, then disable rotate, and repeat.
